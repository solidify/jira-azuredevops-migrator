...
namespace WorkItemImport
{
    public class Agent
    {
        ...
        private void EnsureDateFieldsOnStateChange(WiRevision rev, WorkItem wi)
        {
            ...
            {
                var wiState = wi.Fields[WiFieldReference.State]?.Value.ToString();
                var revState = rev.Fields.FirstOrDefault(f => f.ReferenceName.Equals(WiFieldReference.State, StringComparison.InvariantCultureIgnoreCase))?.Value.ToString();
        ...
        {
            string currentDescription = wi.Type.Name == "Bug" ? wi.Fields[WiFieldReference.ReproSteps].Value.ToString() : wi.Description;
            ...
            if (descUpdated)
            {
                ...
                if (wi.Type.Name == "Bug")
                {
                    wi.Fields[WiFieldReference.ReproSteps].Value = currentDescription;
                }
                else
                {
                    wi.Fields[WiFieldReference.Description].Value = currentDescription;
                }
            }
